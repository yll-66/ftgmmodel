function [x_f,rmseX,rmseX_4_step] = ftgm_4(t,xx,xs,xf,nf,m,r,w)
...
% xf:   forecasting series with length nf 
            ...
            pa_7=0.5*[cos(w*t(1:m-1))+cos(w*t(2:m)) sin(w*t(1:m-1))+sin(w*t(2:m)) cos(2*w*t(1:m-1))+cos(2*w*t(2:m)) sin(2*w*t(1:m-1))+sin(2*w*t(2:m)) cos(3*w*t(1:m-1))+cos(3*w*t(2:m)) sin(3*w*t(1:m-1))+sin(3*w*t(2:m)) cos(4*w*t(1:m-1))+cos(4*w*t(2:m)) sin(4*w*t(1:m-1))+sin(4*w*t(2:m))];
            A=[pa_1 U pa_3 pa_4 pa_5 pa_6 pa_7];
            pa(:)=inv(A'*A)*A'*xx(2:m);
            %% %%%%Origin-based 参数估计%%%%%%%%%%%
            %Z=0.5*Y1(1:m-1)+0.5*Y1(2:m)+0.5*r*X(1);
            Z(:)=0.5*Y1(1:m-1)+0.5*Y1(2:m)+0.5*r*((pa(1)+sum(pa(3:10)))*xx(1)+pa(2)+sum(pa(11:18)));
            ...
            %eta=X(1);
            eta(:) =(pa(1)+sum(pa(3:10)))*xx(1)+pa(2)+sum(pa(11:18));
            X0 = [xx(1) eta(:)];%初始值选择
            %%  %%%%%积分匹配模型拟合误差
            [t,X] = ode45(@(t,X) odefcnnnn(t,X,P,w), t, X0);
            x_f=X(:,2);
            x_fit=x_f(1:m);
            x_fore=x_f(m+1:end);
            ...
            rmseX_4_step = [rmseIn rmseOut_1 rmseOut_6 rmseOut_12 rmseOut];
